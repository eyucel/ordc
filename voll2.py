...
def vollsim(ps, bf_f, bf_s):
    ...
    for c in container_list:
        if c.name == "flat":
            sorted_costs[sorted_costs <.001] = .001
            sorted_costs[sorted_costs >1.98] = 1.98
...
if __name__ == "__main__":
    p = np.linspace(0.01, 1.99, num=199, endpoint=True)
    ...
    k = 3
    # bf_f = solve_opt(n, k)
    # bf_f = honest_bid  # for pabs


    z = np.genfromtxt("n6k_x.csv",delimiter=',')
    print('hi')
    bf_list = [interp1d(z[:,2*i],np.where(z[:,2*i+1]<0, 0, z[:,2*i+1])) for i in range(5)]
    bf_f = bf_list[2]
    ...
    plt.plot(p, num_clear_list[flat], label='pab')
    ...
    f1.savefig("avg_units.pdf",bbox_inches='tight')
    ...
    handles, labels = ax.get_legend_handles_labels()
    ax.legend(handles, labels, loc=0)
    f3.savefig("voll_auction_costs_97_pab.pdf",bbox_inches='tight')
    plt.show()